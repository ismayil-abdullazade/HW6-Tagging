...
class HiddenMarkovModel:
    ...
    def forward_pass(self, isent: IntegerizedSentence) -> TorchScalar:
        ...
        log_alpha[0] = torch.log(alpha[0] + 1e-45)  # add small epsilon to avoid log(0)
        
        # Precompute log matrices for efficiency
        ...
        for j in range(1, len(isent)):
            ...
            # Get emission log-probabilities for this word
            if w_j < self.V:  # regular word
                log_B_col = log_B[:, w_j]
            else:  # EOS_WORD or BOS_WORD - structural emissions
        ...
        log_Z = log_alpha[-1][self.eos_t]  # probability ends at EOS_TAG
        self.log_Z = log_Z
    ...
    def backward_pass(self, isent: IntegerizedSentence, mult: float = 1) -> TorchScalar:
        ...
        for j in range(len(isent) - 2, -1, -1):  # from n down to 0
            ...
            # Get emission log-probabilities for next word
            if w_next < self.V:
    ...
    def viterbi_tagging(self, sentence: Sentence, corpus: TaggedCorpus) -> Sentence:
        ...
        for j in range(1, len(isent)):
            ...
            # For each possible current tag t
